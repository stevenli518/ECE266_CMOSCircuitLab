...
function [sinusoid_power, data_minus_sinusoid_in_BW_power, SNDR, ENOB,HD2,HD3, SNR, SFDR] = ...
    plot_periodogram_SFDR(data, ...
    periodogram_length, ...
    num_segments, ...
    f_signal, ...
    f_s, ...
...
data_spectrum_linear = 10.^(data_spectrum./10);
SNR = snr(data_spectrum_linear,f,'psd');
if HD2 < HD3
    SFDR = 10*log10(sinusoid_power/sinusoid_power_HD2);
else
    SFDR = 10*log10(sinusoid_power/sinusoid_power_HD3);
end
% Plot the PSDs
if plotYN == 1
    if plotAll == 1
